...
import { getProductById, FREE_GIFT } from '../../../lib/products';
...
function mapOrderLineItems(shopifyLineItems) {
  return (shopifyLineItems || []).map((li) => {
    ...
    const catalogProduct = productId === FREE_GIFT.id ? FREE_GIFT : (productId ? getProductById(productId) : null);
    return {
      id: productId || li.variant?.id || 'unknown',
      name: catalogProduct?.name || li.title || 'Unknown item',
      price: catalogProduct?.id === FREE_GIFT.id ? 0 : Number(li.price ?? catalogProduct?.price ?? 0),
