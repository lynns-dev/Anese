...
// requestContact: ask the wallet's own sheet to collect the shopper's
// name/address/email/phone. Off by default — the checkout page already
// collects all of that in Step 1, and asking twice would be worse than
...
export async function createSquarePaymentRequest(amount, label = 'ANESE', { requestContact = false } = {}) {
  const payments = await getSquarePayments();
  const locationId = process.env.NEXT_PUBLIC_SQUARE_LOCATION_ID;
...
// Safari's special -apple-pay-button CSS appearance (see pages/checkout.jsx's
// .apple-pay-button style), and this just returns the object whose
// tokenize() that button's onClick calls directly. Returns null (rather
// than throwing) if this wallet can't be offered here — e.g. Apple Pay
// outside Safari, or before this domain has completed Apple's merchant
// domain verification — so calling pages can just hide the button instead
// of crashing.
...
// on-screen without needing to be at a console. Purely additive; omitting
// it is identical to the old behavior.
export async function createApplePayButton(amount, onError, { requestContact = false } = {}) {
  try {
    const payments = await getSquarePayments();
    const paymentRequest = await createSquarePaymentRequest(amount, 'ANESE', { requestContact });
...
export async function createGooglePayButton(amount, containerId, onError, { requestContact = false } = {}) {
  try {
    const payments = await getSquarePayments();
    const paymentRequest = await createSquarePaymentRequest(amount, 'ANESE', { requestContact });
...
export async function createAfterpayButton(amount, containerId, onError, { requestContact = false } = {}) {
  try {
    const payments = await getSquarePayments();
    const paymentRequest = await createSquarePaymentRequest(amount, 'ANESE', { requestContact });
