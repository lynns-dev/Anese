...
import { FREE_GIFT, FREE_GIFT_THRESHOLD } from './products';

const CartContext = createContext(null);
const STORAGE_KEY = 'anese-cart';
const DISCOUNT_STORAGE_KEY = 'anese-discount';
...
export function CartProvider({ children }) {
  ...
  // Auto-add/remove the free gift as the paying subtotal crosses
  // FREE_GIFT_THRESHOLD — no button, no timer, just there once earned (and
  // taken back out if items are removed and the cart drops back below the
  // threshold, so it can't be kept after the qualifying order shrinks).
  // Silent (no drawer-open, no AddToCart pixel/analytics event) since this
  // isn't a shopper action — it's the cart correcting itself.
  useEffect(() => {
    if (!hydrated) return;
    setCart((prev) => {
      const paidTotal = prev.reduce((s, i) => (i.id === FREE_GIFT.id ? s : s + i.price * i.quantity), 0);
      const hasGift = prev.some((i) => i.id === FREE_GIFT.id);
      const qualifies = paidTotal >= FREE_GIFT_THRESHOLD;
      if (qualifies && !hasGift) {
        return [...prev, { ...FREE_GIFT, price: 0, originalPrice: FREE_GIFT.price, quantity: 1 }];
      }
      if (!qualifies && hasGift) {
        return prev.filter((i) => i.id !== FREE_GIFT.id);
      }
      return prev;
    });
  }, [cart, hydrated]);
