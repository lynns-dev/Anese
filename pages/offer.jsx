...
// dressed in ANESE's own type/color system rather than a generic template
...
const RITUAL_STEPS = [
  ...
  ['02', 'Sweep where you’re noticed', 'Collarbones, shoulders, the backs of the knees. A anese, not a coat.'],
...
const REEL_VIDEOS = [
  ...
  '/videos/anese-ugc-video-1.mp4',
  '/videos/anese-ugc-video-2.mp4',
];
...
export default function OfferPage() {
  ...
  return (
    <div style={{ background: T.white }}>
      <Head>
        <title>ANESE — The Fragrance Ritual That Takes 10 Seconds</title>
      </Head>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '0 24px' }}>
        ...
        <section style={{ paddingTop: 48 }}>
          ...
          <p style={{ fontSize: 17, color: T.soft, marginTop: 18, lineHeight: 1.5 }}>
            Discover why women are trading $200 perfume bottles for one $45 jar of ANESE.
          </p>
        ...
        <section style={{ fontSize: 17, lineHeight: 1.7, color: T.ink }}>
          ...
          <p>So when I heard about <b>ANESE</b>, a scented body powder that promises the wear of a full perfume bottle from a single jar — pressed in with a puff instead of sprayed — I was skeptical. I’d tried powders before. They faded faster than perfume, if they had any scent at all.</p>
          <p><i>Here’s what I found instead…</i></p>
        </section>

        <div style={{ margin: '32px -24px', aspectRatio: '4/3', overflow: 'hidden' }}>
          <img src="/images/anese-ugc-1.webp" alt="A ANESE customer holding her jar" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
        </div>
        ...
        <div style={{ textAlign: 'center', margin: '36px 0' }}>
          <button className="cta-3d" onClick={handleClaim} disabled={claiming} style={{ ...ctaBtn, width: '100%', maxWidth: 420, height: 56, opacity: claiming ? 0.6 : 1 }}>
            {claiming ? 'Loading…' : `Try ANESE Now — 15% off with code ${DISCOUNT_CODE}`}
          </button>
        </div>
        ...
        <section style={{ padding: '8px 0 44px' }}>
          <p style={{ ...S.label, textAlign: 'center', marginBottom: 16 }}>Real women, real Anese</p>
          <div className="ugc-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 3, margin: '0 -24px' }}>
            {['/images/anese-ugc-2.webp', '/images/anese-ugc-3.webp'].map((src) => (
              <div key={src} style={{ aspectRatio: '4/5', overflow: 'hidden' }}>
                <img src={src} alt="A ANESE customer holding her jar" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
              </div>
            ))}
          </div>
        </section>
        ...
        <div style={{ textAlign: 'center', margin: '36px 0' }}>
          <button className="cta-3d" onClick={handleClaim} disabled={claiming} style={{ ...ctaBtn, width: '100%', maxWidth: 420, height: 56, opacity: claiming ? 0.6 : 1 }}>
            {claiming ? 'Loading…' : 'Try ANESE Now →'}
          </button>
        </div>
        ...
        <section style={{ padding: '24px 0' }}>
          <div style={{ border: `1px solid ${T.line}`, background: T.paper, padding: '28px 24px' }}>
            <h3 style={{ fontFamily: T.serif, fontWeight: 400, fontSize: 20, margin: '0 0 16px' }}>Why Women Choose ANESE:</h3>
      ...
      <footer style={{ borderTop: `1px solid ${T.line}`, padding: '28px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 11, color: T.soft }}>&copy; {new Date().getFullYear()} ANESE. All rights reserved.</p>
      </footer>
